...
func (m *UserMutation) SetUILanguage(ul user.UILanguage) {
	m.ui_language = &ul
}

// UILanguage returns the value of the "ui_language" field in the mutation.
func (m *UserMutation) UILanguage() (r user.UILanguage, exists bool) {
	v := m.ui_language
	if v == nil {
		return
	}
...
// OldUILanguage returns the old "ui_language" field's value of the User entity.
// If the User object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *UserMutation) OldUILanguage(ctx context.Context) (v user.UILanguage, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldUILanguage is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldUILanguage requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldUILanguage: %w", err)
	}
	return oldValue.UILanguage, nil
}

// ResetUILanguage resets all changes to the "ui_language" field.
func (m *UserMutation) ResetUILanguage() {
	m.ui_language = nil
}
...
func (m *UserMutation) Fields() []string {
	...
	if m.ui_language != nil {
		fields = append(fields, user.FieldUILanguage)
	}
	if m.admin != nil {
